...
> ✏️ **현행 갱신 (2026-09-10, 저자 지시 4단계 진행)**: P4 검토·프로즈 수정(`7df3e6a`), C-8 초록 재구성(`94df01e`), App Note 전환(`docs/2026-09-10-appnote/`) 완료. 긴 원문은 `manuscript_full.md`, 19개 표와 상세 근거는 `supplementary.md`, 제출 후보 PDF/DOCX는 `submission/`. 다음은 정확한 검증 커밋의 **0.9.0 태그·draft 릴리스 → Zenodo 실제 발급 → 초록 URL·최종 제출본 갱신 → 저자 제출 승인**. Zenodo 연동/토큰, 저자 선언, 현행 OUP 규정 확인이 남았다. DOI나 제출 완료를 추정하지 않는다. 아래 Phase 문장은 앞선 순서의 이력이다.
